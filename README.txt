...
3) Open cmd and go to sbt prompt by giving command 'sbt' (eg: $path\Project\Server>sbt)
4) To execute the server, give command "run-main BitcoinServer k" where k is no. of leading zeros (eg: $path\Project\Server>run-main BitcoinServer 3)
...
3) Open cmd and go to sbt prompt by giving command 'sbt' ($path\Project\Client>sbt)
4) To execute the client, give command "run-main BitcoinClient ipaddress"($path\Project\Client>run-main BitcoinClient 192.168.1.3)
...
4) The coins with the most 0s we managed to find were 7
